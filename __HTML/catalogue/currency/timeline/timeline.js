    ...
    while (yearsLeft > 0) {
        ...
        while (totalYears < numCols) {
            firstHeaderHTML += `<th colspan="${Math.min(3 * (issueYears[localSeriesIndex].length - seriesYearIndex),3*(MAX_NUM_YEARS - totalYears))}">
                                    <a href="/catalogue/currency/index.html?currencyId=${currencyId}&seriesId=${notesArray[localSeriesIndex].seriesId}">
...
function openUpsertCollection() {
    if (Session.getUsername()) {
        ...
        new UpsertCollectionForm(variantJSON, gradesJSON, () => { initializeTimeline() });
    } else
