function initializeUpsertCollection(seriesId, variantJSON, gradesJSON) {
    ...
    variantJSON.items.forEach(item => {
        ...
        itemRow += `             </select>
                            ...
                            <td rowspan="2"><img src="/catalogue/currency/forms/collection/delete.png" alt="delete item" onclick="deleteCollectionRow(this)"></td>
                        </tr>
                        <tr>
...
function submitItems() {
    ...
    let menuOpt = $("#currency-views>p.selected-view").text();
    if (menuOpt === "")
        menuOpt = "Country." + $("#country-views>p.selected-view").text();
    switch (menuOpt) {
        case "Timeline":
            initializeTimeline();
            break;
